...
func (c *MenuBarComponent) Draw(r core.Renderer) {
	...
	// Engine version, pinned to the far right of the toolbar. The editor is rebuilt
	// alongside the engine, so the two always match (engineVersion is generated by
	// `imge build` into this package).
	ver := "imge " + engineVersion
	vw, _ := r.MeasureText(ver, c.FontID, c.FontSize)
	verX := rect.X() + rect.Width() - vw - 8
	r.DrawText(ver, c.FontID, c.FontSize, math.NewVector2(verX, cy), c.Dim)

	// Status line, right-aligned just left of the version.
	if c.status != "" {
		sc := c.Success
		if c.statusError {
			sc = c.Error
		...
		r.DrawText(c.status, c.FontID, c.FontSize, math.NewVector2(verX-sw-12, cy), sc)
	}
...
func (c *MenuBarComponent) save() {
	...
	c.status = "saved"
	c.statusError = false
	console.Print("saved " + vp.SceneFile())
}
...
func (c *MenuBarComponent) run() {
	...
	// Auto-save the loaded scene first so RUN reflects in-progress edits. The save is
	// logged so a RUN's output shows the whole story (save → build → run).
	if err := vp.Save(); err != nil {
		c.status = "error: " + err.Error()
		c.statusError = true
		return
